...
//        configuration.setKeyStrategy(new RandomKeyStrategy());
    ...
    public void test_chat_completions_stream() throws InterruptedException, JsonProcessingException {
        ...
        DefaultChatCompletionRequest defaultChatCompletionRequest = DefaultChatCompletionRequest.builder().stream(true)// 开启流式返回
                .messages(Collections.singletonList(DefaultMessage.builder().role(Constants.Role.USER.getRoleName()).content("讲一个笑话").build())).model(BaseChatCompletionRequest.Model.GPT_3_5_TURBO.getModuleName()).build();
